...
final class SMTP_Fleet_REST {
	...
	public function __construct( SMTP_CRM_Repository $repository, SMTP_Fleet_Calculator $calculator, SMTP_Fleet_Notifications $notifications, SMTP_Fleet_Report $report ) {
	...
	public function routes(): void {
		foreach ( array( 'source-more/v1', 'source-more/v2' ) as $namespace ) {
			register_rest_route( $namespace, '/lead', array( 'methods' => 'POST', 'callback' => array( $this, 'create_lead' ), 'permission_callback' => '__return_true' ) );
		}
	}
	...
	public function create_lead( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( ! wp_verify_nonce( $request->get_header( 'X-WP-Nonce' ), 'wp_rest' ) ) {
			return new WP_Error( 'bad_nonce', SMTP_I18n::text( 'Security check failed.', 'فشل التحقق الأمني.', $lang ), array( 'status' => 403 ) );
		}
		if ( ! SMTP_Rate_Limiter::check( 'lead', 5, 900 ) ) {
			return new WP_Error( 'rate_limited', SMTP_I18n::text( 'Too many submissions. Please try again later.', 'تم إرسال عدد كبير من الطلبات. يرجى المحاولة لاحقًا.', $lang ), array( 'status' => 429 ) );
		}
		...
		SMTP_Logger::info( 'Fleet lead created', array( 'lead_id' => $lead_id, 'source' => $values['lead_source'], 'language' => $lang ) );
		...
		return new WP_REST_Response(
			array(
				...
				'message'        => SMTP_I18n::text( 'Your report is ready.', 'تقريرك جاهز.', $lang ),
			),
			201
		);
	}
