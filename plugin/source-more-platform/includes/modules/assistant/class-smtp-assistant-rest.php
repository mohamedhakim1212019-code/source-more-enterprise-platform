...
final class SMTP_Assistant_REST {
	...
	public function __construct( SMTP_Assistant_Service $service, SMTP_Assistant_Lead_Converter $lead_converter ) {
		$this->service        = $service;
		$this->lead_converter = $lead_converter;
	}
	...
	public function reply( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		$security = $this->security( $request, 'assistant', 20, 600 );
		...
		$result = $this->service->reply(
			...
			absint( $data['conversation_id'] ?? 0 ),
			sanitize_text_field( $data['conversation_token'] ?? '' )
		);
	...
	public function capture_lead( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		$security = $this->security( $request, 'assistant_lead', 5, 900 );
		if ( is_wp_error( $security ) ) {
			return $security;
		}
		...
		$data   = $request->get_json_params();
		$data   = is_array( $data ) ? $data : $request->get_params();
		$result = $this->lead_converter->convert( $data );

		return is_wp_error( $result ) ? $result : new WP_REST_Response( $result, 201 );
	}

	private function security( WP_REST_Request $request, string $bucket, int $limit, int $window ): bool|WP_Error {
		if ( ! wp_verify_nonce( $request->get_header( 'X-WP-Nonce' ), 'wp_rest' ) ) {
			return new WP_Error( 'bad_nonce', 'Security check failed.', array( 'status' => 403 ) );
		}

		if ( ! SMTP_Rate_Limiter::check( $bucket, $limit, $window ) ) {
