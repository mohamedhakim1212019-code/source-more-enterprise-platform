...
final class SMTP_Assistant_Service {
	...
	public function __construct(
		SMTP_Assistant_Knowledge $knowledge,
		SMTP_Assistant_Endpoint $endpoint,
		SMTP_Assistant_Conversations $conversations
	) {
	...
	/**
	 * Generate a response and persist the conversation when enabled.
	 *
	 * @return array<string,mixed>|WP_Error
	 */
	public function reply( string $question, int $conversation_id = 0, string $conversation_token = '' ): array|WP_Error {
		$question = trim( wp_strip_all_tags( $question ) );
		if ( '' === $question || $this->length( $question ) > 500 ) {
			return new WP_Error( 'validation', 'Please enter a valid question.', array( 'status' => 422 ) );
		}
		...
		if ( ! empty( $options['assistant_conversation_logging'] ) ) {
			...
			$this->conversations->add_message( $identity['conversation_id'], 'user', $question, 'visitor' );
		}

		$answer = $this->endpoint->reply( $question, (int) $identity['conversation_id'] );
		if ( null === $answer ) {
			$answer = $this->knowledge->answer( $question );
		}

		$reply = $this->substr( trim( wp_strip_all_tags( (string) $answer['reply'] ) ), 0, 2500 );
		if ( '' === $reply ) {
			$reply = 'Please contact Source More Technology for a detailed assessment.';
		}
