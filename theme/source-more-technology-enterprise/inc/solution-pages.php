...
function smt_render_solution_page( $slug ) {
  ...
  $s = $catalog[$slug];
  $visual_key = function_exists('smt_solution_visual_key') ? smt_solution_visual_key($slug) : 'solutions_hero';
  ?>
  <main class="solution-detail-page">
    <section class="ui-page-hero solution-detail-hero smt-photo-hero"><div class="container solution-hero-grid"><div class="smt-photo-hero-copy">
      <h1><?php echo esc_html($s['title']); ?></h1><p><?php echo esc_html($s['tagline']); ?></p>
    ...
    </div><div class="smt-photo-panel solution-hero-photo"><?php smt_visual_picture($visual_key, $s['title'], ['loading'=>'eager','fetchpriority'=>'high','class'=>'smt-visual-image']); ?><div class="solution-hero-icon"><i class="fa-solid <?php echo esc_attr($s['icon']); ?>"></i></div></div></div></section>
